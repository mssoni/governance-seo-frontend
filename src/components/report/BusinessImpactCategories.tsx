import type { CategoryInsight, Issue, Severity } from '../../types/api'
...
const CATEGORIES: CategoryConfig[] = [
  { name: 'Trust & Credibility', icon: ICON_MAP.shield },
  { name: 'Search Visibility', icon: ICON_MAP.search },
  { name: 'User Experience', icon: ICON_MAP.user },
  { name: 'Operational Risk', icon: ICON_MAP.alert },
]
...
// CHG-018: Personalized rendering when categoryInsights are available
function PersonalizedCards({ insights }: { insights: CategoryInsight[] }) {
  return (
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {insights.map((insight) => {
          ...
          return (
            ...
            >
              {/* Category header with segment-specific display name */}
...
// Legacy rendering when no categoryInsights (backward compat)
function LegacyCards({ issues }: { issues: Issue[] }) {
...
export default function BusinessImpactCategories({
  ...
  // CHG-018: Use personalized rendering when category insights are available
  if (categoryInsights && categoryInsights.length > 0) {
    return <PersonalizedCards insights={categoryInsights} />
  }

  return <LegacyCards issues={issues} />
}
